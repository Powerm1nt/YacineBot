...
export async function analyzeMessageIntent(messageContent) {
  try {
    if (!messageContent || messageContent.trim() === '') {
      console.log('[AnalysisService] Contenu vide, pas d\'analyse d\'intention possible');
      return { intentType: 'NONE' };
    }

    console.log(`[AnalysisService] Analyse d'intention pour le message: "${messageContent.substring(0, 50)}${messageContent.length > 50 ? '...' : ''}"`);
...
IMPORTANT: N'utilise PAS de bloc de code markdown (\`\`\`) dans ta réponse, renvoie uniquement l'objet JSON brut.`;
    ...
    let response;
    if (isUsingDeepSeekAPI()) {
      console.log('[AnalysisService] Utilisation de l\'API DeepSeek pour l\'analyse d\'intention');
    ...
    console.log('[AnalysisService] Réponse d\'analyse d\'intention reçue');
    ...
    if (!result || !result.intentType) {
      console.error('[AnalysisService] Format de réponse d\'intention invalide:', response.output_text);
      return { intentType: 'NONE' };
    }

    console.log(`[AnalysisService] Intention détectée: ${result.intentType}${result.data ? `, Données: ${JSON.stringify(result.data)}` : ''}`);
    return result;
  } catch (error) {
